"""Builds a llama-index VectorStore that mirrors the legacy pipeline's data."""
...
import chromadb
try:  # chromadb <=1.3
    from chromadb.errors import NotFoundError as _ChromaNotFound
...
def _fetch_documents(
    settings: Settings, doc_ids: Sequence[int] | None = None
...
def _filter_new_nodes(
    nodes: Sequence[BaseNode], existing_doc_ids: set[str]
) -> Tuple[List[BaseNode], set[str]]:
...
def _ingest_documents(
    ...
    *,
    ...
    if requested_doc_ids:
        ...
        nodes, skipped_docs = _filter_new_nodes(nodes, existing_doc_ids)

    if not nodes:
        logger.info("No new nodes detected; loading index from vector store only.")
...
def run_ingest(argv: Sequence[str] | None = None) -> None:
    ...
    print(
        "读取 %d 篇文档，解析出 %d 个节点。"
        % (stats["requested_documents"], stats["generated_nodes"])
    )
    ...
    print("当前集合：knowledge_base，总节点数 ≈ %d" % len(index.docstore.docs))
