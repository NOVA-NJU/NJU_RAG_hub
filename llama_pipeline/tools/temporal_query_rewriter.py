"""Temporal-aware query rewrites to bridge date ranges and fuzzy references."""
...
_CHINESE_MONTH_DAY_PATTERN = re.compile(r"([零〇一二两三四五六七八九十]{1,3})月([零〇一二两三四五六七八九十]{1,3})[日号天]?")


class TemporalQueryRewriter:
    """Builds additional retrieval queries based on fuzzy temporal references."""
    ...
    def _find_date_substring(query: str) -> Optional[str]:
        for pattern in (_FULL_DATE_PATTERN, _MONTH_DAY_PATTERN, _MONTH_DAY_CN_PATTERN, _CHINESE_MONTH_DAY_PATTERN):
