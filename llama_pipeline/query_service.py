"""LlamaIndex-backed workflow that mirrors the legacy agent's behavior."""
...
from .ingest import build_embedding, build_storage_context
from .tools.keyword_tool import extract_keywords
from .tools.task_intent_tool import (
    fallback_task_queries,
    generate_task_queries,
    looks_like_task_question,
)
from .tools.temporal_query_rewriter import TemporalQueryRewriter
...
class LlamaRAGService:
    """Drop-in alternative to the legacy RAGAgent using llama-index under the hood."""
    ...
    def _ensure_llm_client(self) -> OpenAI:
        if not self.settings.llm.api_key:
            raise ValueError("LLM_API_KEY is required for the Llama workflow")
    ...
    def _run_pipeline(self, question: str, enforce_time: bool) -> Dict[str, Any]:
        ...
        if looks_like_task_question(question):
            try:
                task_queries = generate_task_queries(
                    question,
                    llm_client=llm_client,
                    model=self.settings.llm.model,
                )
            except Exception as exc:  # pragma: no cover - depends on external LLM
                logger.warning("Task intent expansion failed: %s", exc)
            if not task_queries:
                task_queries = fallback_task_queries(question)
    ...
    def query_with_time_focus(self, question: str) -> Dict[str, Any]:
        return self.query(question, enforce_time=True)

    async def async_query(self, question: str, enforce_time: bool = False) -> Dict[str, Any]:
        return await asyncio.to_thread(self.query, question, enforce_time)

    async def async_query_with_time_focus(self, question: str) -> Dict[str, Any]:
        return await self.async_query(question, enforce_time=True)
